...
const Navbar = () => {
  ...
  return (
    <>
      <header className='flex justify-between sticky top-0 p-6 bg-white shadow-sm items-center bg-primary'>
        <h2 className='cursor-pointer uppercase font-extrabold text-2xl ml-8 text-white'>
          <Link to="/"> Task Manager </Link>
        </h2>
        <ul className='hidden md:flex gap-4 uppercase font-medium'>
          {authState.isLoggedIn ? (
            <>
              <li className="bg-blue-500 text-white hover:bg-blue-600 font-bold rounded-md">
                <Link to='/tasks/add' className='block w-full h-full px-4 py-2'> <i className="fa-solid fa-plus"></i> Add task </Link>
              </li>
              <li className='py-2 px-3 cursor-pointer text-white hover:bg-gray-200 hover:text-primary transition rounded-sm font-bold' onClick={handleLogoutClick}>Logout</li>
            </>
          ) : (
            <li className='mr-8 py-2 px-3 font-bold cursor-pointer text-white hover:bg-gray-100 hover:text-primary transition rounded-sm'><Link to="/login">Login</Link></li>
          )}
        </ul>
